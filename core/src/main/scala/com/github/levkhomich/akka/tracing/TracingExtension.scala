...
class TracingExtensionImpl(system: ActorSystem) extends Extension {
  ...
  private[tracing] val holder = {
    ...
    if (config.hasPath(AkkaTracingHost) && (!config.hasPath(AkkaTracingEnabled) || config.getBoolean(AkkaTracingEnabled))) {
      ...
      system.actorOf(Props(new SpanHolder(config.getInt(AkkaTracingSampleRate), transport)), "spanHolder")
    } else {
  ...
  def createChildSpan(spanId: Long, ts: BaseTracingSupport): Unit =
